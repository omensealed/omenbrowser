...
`reticulum_instance_mode = "managed"` is the supported v0.10.0-1 product mode.
...
Official `reticulum-rs-transport 0.10.0` passes OMEN's strengthened
...
The published 0.10.0 sender drops TTL, idempotency, correlation, and extensions
...
the integrated v0.10.0-1 runtime, select Managed and restart. Do not delete
