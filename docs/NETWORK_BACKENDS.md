...
`reticulum_instance_mode = "managed"` is the supported v0.9.9-2 product mode.
...
Official `reticulum-rs-transport 0.9.9` passes OMEN's strengthened
...
The published 0.9.9 sender drops TTL, idempotency, correlation, and extensions
...
the integrated v0.9.9-2 runtime, select Managed and restart. Do not delete
