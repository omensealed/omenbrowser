...
### v0.6.0-1 / v0.9.6-7 compatibility boundary

The application release number does not version the OMENchat wire protocol.
The v0.9.6-7 release retains protocol version `1`, protocol name
...
terminal, link close, shutdown, or six-hour TTL. The pinned 0.9.6 inbound
...
`server-headless`/`server-full` server at 0.9.6-7. Capability names come from
