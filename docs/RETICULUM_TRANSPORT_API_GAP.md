...
and executable-form exchanges pass against current Python RNS 1.4.2 and
NomadNet 1.2.8.
...
Typed queue occupancy/capacity/drop, traffic, violations/filter hits, active
Links, and medium timeout are mapped into bounded/redacted project-owned
snapshots. Unknown remains different from zero. These values are diagnostics
and reconnect/load evidence only, never delivery or durable commit evidence.

## Split metadata Resource boundary remains resolved on 0.10.0

Official `reticulum-rs-transport 0.10.0` retains the correction for upstream
issue #553 / PR #556. The unchanged registry sentinel passed first. Because
0.10.0 may auto-compress repetitive data, the promoted regression additionally
...
### Routed Resource retransmission boundary on 0.10.0
...
The exact registry `reticulum-rs-transport 0.10.0` duplicate filter admits
...
`docs/upstream/reticulum-rs-0.10.0-routed-resource-retransmission.md`. The
independent UDP boundary is documented separately in
`docs/upstream/reticulum-rs-0.10.0-udp-max-wire-buffer.md`.
...
The 0.10.0 transport library exposes public `Link::request_packet` and
...
The 0.10.0 crate also exposes public request/response Resource helpers.
...
Published `reticulum-rs-transport` 0.10.0 still exposes IFAC-related shared
...
Reticulum 0.10.0 requalification preserves both boundaries independently. The
