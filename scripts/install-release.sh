...
usage() {
  cat <<'USAGE'
usage: bash scripts/install-release.sh [options]

Install or remove the optional public release user integrations.

Default install:
  - installs the isolated OMENbrowser_rs release desktop launcher only
...
Path options:
  ...
  --browser-root DIR         isolated release browser app root
  --browser-root-2 DIR       second isolated release browser app root
...
omenchatd service not installed. To add it later:
  bash scripts/install-release.sh --server-service
EOF
fi
