...
grep -q "chat-client-reticulum:on" "${out_root%/}/omenbrowser_rs-package-version.txt"
grep -q "native-network:on" "${out_root%/}/omenbrowser_rs-package-version.txt"
grep -q "desktop-product:on" "${out_root%/}/omenbrowser_rs-package-version.txt"
grep -q "mock-runtime:off" "${out_root%/}/omenbrowser_rs-package-version.txt"
...
install -m 0644 assets/fonts/adwaita/OFL.txt "$pkg_dir/usr/share/doc/omenbrowser-rs/ADWAITA_MONO_OFL.txt"
