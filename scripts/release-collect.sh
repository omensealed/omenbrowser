...
browser_root="${OMENBROWSER_RELEASE_ROOT:-}"
browser_root_2="${OMENBROWSER_RELEASE_ROOT_2:-}"
server_home="${OMENCHATD_RELEASE_HOME:-}"
out_root="${TMPDIR:-/tmp}/omenbrowser-rs-release-bundles"
tail_lines=400
unit_name="${OMENCHATD_UNIT:-omenchatd}"

usage() {
  cat <<'USAGE'
usage: bash scripts/release-collect.sh [options]

Collect a redacted issue bundle without copying private identity material,
...
bundle_dir="${out_root%/}/release-${timestamp}"
mkdir -p "$bundle_dir"
...
write_root_sanity() {
  ...
  local helper="scripts/release-root-sanity.sh"
...
Attach this directory when reporting release test failures.
