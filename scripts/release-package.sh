...
cp scripts/release-root-sanity.sh "$target_dir/scripts/"
cp scripts/install-release.sh "$target_dir/scripts/"
...
cat > "$target_dir/START.txt" <<'EOF'
OMENbrowser_rs public release bundle
...
Run the OMENchat server:
  ...
  ./bin/omenchatd run --home /tmp/omenchatd-test
...
Optional combined release installer:
...
selfcheck_home="$(mktemp -d "${TMPDIR:-/tmp}/omenchatd-release-package-selfcheck.XXXXXX")"
...
(
  ...
  sha256sum \
    ...
    scripts/release-root-sanity.sh \
...
extract_root="$(mktemp -d "${TMPDIR:-/tmp}/omenbrowser-release-package-extract.XXXXXX")"
...
test -f "$extracted_dir/scripts/release-collect.sh"
test -f "$extracted_dir/scripts/release-omenchat-smoke.sh"
test -f "$extracted_dir/scripts/release-root-sanity.sh"
test -f "$extracted_dir/scripts/install-release.sh"
...
bash -n "$extracted_dir/scripts/release-omenchat-smoke.sh"
bash -n "$extracted_dir/scripts/release-root-sanity.sh"
bash -n "$extracted_dir/scripts/install-release.sh"
...
bash "$extracted_dir/scripts/release-root-sanity.sh" \
...
extracted_home="$(mktemp -d "${TMPDIR:-/tmp}/omenchatd-release-package-extracted.XXXXXX")"
...
echo "== Updating latest release package copy =="
latest_archive="${out_root%/}/OMENbrowser_rs-latest.tar.gz"
latest_manifest="${out_root%/}/OMENbrowser_rs-latest.txt"
