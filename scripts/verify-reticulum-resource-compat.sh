...
if ! grep -Eq '^rns_transport[[:space:]]*=.*version[[:space:]]*=[[:space:]]*"=0\.9\.7"' Cargo.toml \
  || ! grep -Eq '^rns_transport[[:space:]]*=.*version[[:space:]]*=[[:space:]]*"=0\.9\.7"' src/server/Cargo.toml; then
  echo "Resource compatibility guard applies only to the exact Reticulum 0.9.7 train" >&2
  exit 1
fi
...
echo "Reticulum 0.9.7 Resource compatibility guards: pass"
