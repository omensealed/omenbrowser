...
routed_doc="${OMEN_RETICULUM_ROUTED_DOC:-docs/upstream/reticulum-rs-0.10.0-routed-resource-retransmission.md}"
udp_doc="${OMEN_RETICULUM_UDP_DOC:-docs/upstream/reticulum-rs-0.10.0-udp-max-wire-buffer.md}"
...
if ! grep -Eq '^rns_transport[[:space:]]*=.*version[[:space:]]*=[[:space:]]*"=0\.10.0"' Cargo.toml \
  || ! grep -Eq '^rns_transport[[:space:]]*=.*version[[:space:]]*=[[:space:]]*"=0\.10.0"' src/server/Cargo.toml; then
  echo "Resource compatibility rebaseline requires the exact Reticulum 0.10.0 train" >&2
  exit 1
fi
...
test_block "$split" | grep -Fq 'upstream split-metadata regression sentinel failed on official 0.10.0' ||
  fail "split-metadata exact-train assertion is missing"

attribute_block "$udp" | grep -Fq '#[ignore = "known upstream Reticulum 0.10.0 UDP maximum-Resource serialization regression"]' ||
...
attribute_block "$routed" | grep -Fq '#[ignore = "known upstream Reticulum 0.10.0 routed Resource retransmission regression; requires the documented three-node fragment-loss topology"]' ||
...
grep -Fq '`reticulum-rs-transport 0.10.0`' "$routed_doc" ||
...
grep -Fq '`reticulum-rs-transport 0.10.0`' "$udp_doc" ||
...
echo "Reticulum 0.10.0 Resource compatibility: split regression normal; routed and maximum-UDP sentinels protected"
