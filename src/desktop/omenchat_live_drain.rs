    ...
    pub(in crate::desktop) fn drain_omenchat_runtime_events(&mut self) -> Task<Message> {
        ...
        for terminal in self.app.drain_omenchat_resource_terminals() {
            ...
            let status = if matches!(
                ...
                format!(
                    "OMENchat transfer failed on the current route; attachment was not committed; upstream 0.10.0 routed Resource retransmission limits may apply; released {released} pending offer(s); no automatic retry occurred"
                )
            } else {
                format!(
